...
ImportService _service(BackupService backup) => ImportService(
  backup: backup,
...
void main() {
  ...
  group('Aegis (plain v1) end to end', () {
    test(
      'detected and parsed through the real parser + real dedupe key',
      () async {
    ...
    test(
      'a token already in the vault is reported as alreadyInVault',
      () async {
        final raw = _fixture('aegis_plain_v1.json');
  ...
  group('issuer canonicalization end to end', () {
    ...
    ImportService catalogService() => ImportService(
      ...
      canonicalizeResolver: () => canonicalizerFor(
        IssuerCatalog(const [
          CatalogService(
            id: '1',
            name: 'GitHub',
    ...
    test('vault "GitHub" + file "github.com" → alreadyInVault', () async {
      final preview = await catalogService().preview(
        raw: aliasFile(),
        existing: [inVault('GitHub')],
      );

      expect(preview.addCount, 0);
      expect(
        _countOf(preview, SkipReason.alreadyInVault),
        1,
        reason:
            'VaultCubit stores "GitHub"; a re-import of the alias file '
    ...
    test(
      'an entry the catalog does not know keeps its issuer verbatim',
      () async {
        final preview = await catalogService().preview(
          raw: _fixture('aegis_plain_v1.json'),
          existing: const [],
  ...
  group('export → import round trip', () {
    ...
    test(
      'our own backup is detected, decrypted and fully restorable',
      () async {
        final raw = await backup.export(
          accounts: accounts,
    ...
    test(
      'a wrong backup password fails as WrongBackupPasswordException',
      () async {
        final raw = await backup.export(
          accounts: accounts,
          password: _backupPassword,
  ...
  group('Google Authenticator migration end to end', () {
    ...
    List<String> threeCodes({int batchId = 77}) {
      final alice = _entry(
        secretHex: aliceSecretHex,
        name: 'alice@example.com',
        issuer: 'Example',
      );
      final bob = _entry(
        secretHex: bobSecretHex,
        name: 'bob@example.com',
        issuer: 'ACME',
      );
      final aliceAgain = _entry(
        secretHex: aliceSecretHex,
        name: 'alice@example.com',
        issuer: 'Example',
      );
      return <String>[
        for (final (index, entry) in <Uint8List>[
          alice,
          bob,
          aliceAgain,
        ].indexed)
    ...
    test(
      'a single-code export goes from raw QR string to a one-token preview',
      () {
        // Real parser: URI → base64 → protobuf → OtpAccount, no stubs anywhere.
        expect(
          controller.handleRaw(_vectorAUri),
          const MigrationScanComplete(1, 1),
        );
    ...
    test('three codes scanned out of order stitch back together, and the '
      ...
      expect(preview.toAdd.map((a) => a.accountName), <String>[
        'alice@example.com',
        'bob@example.com',
      ]);
    });
  ...
  group('tags end to end (grouped Aegis → vault → backup → re-import)', () {
    test(
      'group labels reach the vault and come back out of our own backup',
      () async {
  ...
  test(
    'an unrecognized JSON object is refused before any parser runs',
    () async {
  ...
  test('text that is not JSON is refused before fingerprinting', () {
    expect(
      () => service.detect('not json at all'),
      throwsA(isA<MalformedImportFileException>()),
    );
  });

  test(
    'a structurally broken Aegis file is refused by the real parser',
    () async {
