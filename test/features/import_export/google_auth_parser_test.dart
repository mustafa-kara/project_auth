...
const String _vectorAUri = 'otpauth-migration://offline?data='
...
const String _vectorBRawUri = 'otpauth-migration://offline?data='
    'ChcKCj4/+++++gARIjMSA2JvYiABKAEwAhABGAEgACj+//////////8B';
const String _vectorBEncodedUri = 'otpauth-migration://offline?data='
...
void main() {
  group('looksLikeMigrationUri', () {
    test('accepts the migration scheme, case-insensitively and with padding',
        () {
    ...
    test('rejects a plain otpauth:// token and unrelated text', () {
      expect(
          ...
          isFalse);
      expect(GoogleAuthParser.looksLikeMigrationUri('https://example.com'),
          isFalse);
  ...
  group('golden vector A', () {
    test('the encoder reproduces the plan payload byte for byte', () {
      final built = encodeMigrationPayload(entries: [
    ...
    test('the pinned hex payload decodes identically to the pinned URI', () {
      final fromBytes =
          GoogleAuthParser.parsePayload(fromHex(_vectorAPayloadHex));
      ...
      expect(fromBytes.accounts.single.accountName,
  ...
  group('golden vector B — negative batch_id and the "+" trap', () {
    test('the encoder reproduces the plan payload byte for byte', () {
      final built = encodeMigrationPayload(
        entries: [
          _entry(
            secret: fromHex(_vectorBSecretHex),
            name: 'bob',
          ),
        ],
        batchId: -2,
    ...
    test('the raw and the percent-encoded URI give the identical result', () {
      ...
      for (final batch in [raw, encoded]) {
        ...
        expect(batch.accounts.single.secret,
    ...
    test(
      ...
      final viaQueryParameters =
          Uri.parse(_vectorBRawUri).queryParameters['data'];
      expect(viaQueryParameters, contains(' '));

      final parsed = GoogleAuthParser.parseUri(_vectorBRawUri);
      expect(parsed.accounts.single.secret,
          Base32.encode(fromHex(_vectorBSecretHex)));
    });
  });
  ...
  group('URI validation', () {
    test('a wrong scheme is refused', () {
      expect(
          () => GoogleAuthParser.parseUri(
              'otpauth://offline?data=Ci4KCkhlbGxv'),
    ...
    test('a missing, empty or blank data parameter is refused', () {
      for (final uri in const [
        ...
        expect(() => GoogleAuthParser.parseUri(uri),
            throwsA(isA<MalformedMigrationUriException>()),
            reason: uri);
      }
    });
    ...
    test('undecodable base64 is refused', () {
      ...
      expect(
          () => GoogleAuthParser.parseUri(
              'otpauth-migration://offline?data=AAAAA'),
          throwsA(isA<MalformedMigrationUriException>()));
    });
    ...
    test('a URI over maxUriLength is refused before anything is decoded', () {
      final huge = 'otpauth-migration://offline?data='
          '${'A' * (GoogleAuthParser.maxUriLength + 1)}';
      expect(() => GoogleAuthParser.parseUri(huge),
          throwsA(isA<MalformedMigrationUriException>()));
    });
    ...
    test('EVERY truncated prefix of a real payload fails cleanly', () {
      final payload = encodeMigrationPayload(
        entries: [_entry(issuer: 'Example'), _entry(name: 'bob')],
        batchSize: 2,
        batchIndex: 1,
        batchId: -2,
      );
  ...
  group('batch coordinates', () {
    test('absent batch fields fall back to the proto3 defaults', () {
      final batch = GoogleAuthParser.parsePayload(encodeMigrationPayload(
        entries: [_entry()],
        version: null,
        batchSize: null,
        batchIndex: null,
    ...
    test('more than maxEntriesPerBatch entries is a FormatException', () {
      final entries =
          List<Uint8List>.generate(GoogleAuthParser.maxEntriesPerBatch + 1,
  ...
  group('mapping — algorithm', () {
    test('UNSPECIFIED and SHA1 both map to SHA1', () {
      for (final value in const [
        ...
        expect(batch.accounts.single.algorithm, OtpAlgorithm.sha1,
            reason: 'algorithm=$value');
      }
    });

    test('SHA256 and SHA512 map through', () {
      expect(_parse([_entry(algorithm: ProtoAlgorithm.sha256)])
          .accounts
          .single
          .algorithm,
          OtpAlgorithm.sha256);
      expect(_parse([_entry(algorithm: ProtoAlgorithm.sha512)])
          .accounts
          .single
          .algorithm,
          OtpAlgorithm.sha512);
  ...
  group('mapping — digits', () {
    test('UNSPECIFIED and SIX map to 6, EIGHT maps to 8', () {
      expect(_parse([_entry(digits: ProtoDigits.unspecified)])
          .accounts
          .single
          .digits,
          6);
      expect(_parse([_entry(digits: ProtoDigits.six)]).accounts.single.digits, 6);
      expect(
          _parse([_entry(digits: ProtoDigits.eight)]).accounts.single.digits, 8);
    });
  ...
  group('mapping — type', () {
    ...
    test('HOTP with an explicit counter maps to hotp', () {
      final account =
          _parse([_entry(type: ProtoOtpType.hotp, counter: 42)]).accounts.single;
    ...
    test('UNSPECIFIED is unsupportedType — never guessed as TOTP', () {
      final skip =
          _onlySkip(_parse([_entry(type: ProtoOtpType.unspecified)]));
      expect(skip.reason, SkipReason.unsupportedType);
    ...
    test('an issuer of "Steam" is NOT promoted to a Steam token', () {
      ...
      final account =
          _parse([_entry(issuer: 'Steam', name: 'gaben')]).accounts.single;
      expect(account.type, OtpType.totp);
      expect(account.digits, 6);
  ...
  group('mapping — HOTP counter presence', () {
    ...
    test('a negative counter is invalidFields', () {
      final skip =
          _onlySkip(_parse([_entry(type: ProtoOtpType.hotp, counter: -1)]));
      expect(skip.reason, SkipReason.invalidFields);
    ...
    test('a counter on a TOTP entry is ignored, not carried over', () {
      final account =
          _parse([_entry(type: ProtoOtpType.totp, counter: 99)]).accounts.single;
      expect(account.type, OtpType.totp);
      expect(account.counter, 0);
    });
  });
  ...
  group('mapping — name and issuer', () {
    test('the issuer field wins over the one embedded in the label', () {
      final account =
          _parse([_entry(name: 'Label:alice', issuer: 'Field')]).accounts.single;
    ...
    test('a name without a colon is entirely the account name', () {
      final account = _parse([_entry(name: 'alice@example.com')]).accounts.single;
    ...
    test('surrounding whitespace is trimmed off both halves', () {
      final account =
          _parse([_entry(name: '  GitHub :  alice  ')]).accounts.single;
      expect(account.issuer, 'GitHub');
      expect(account.accountName, 'alice');
    });
    ...
    test('an empty name falls back to the issuer', () {
      final account =
          _parse([_entry(name: '', issuer: 'GitHub')]).accounts.single;
      expect(account.issuer, 'GitHub');
      expect(account.accountName, 'GitHub');
    });

    test('an entry with neither name nor issuer gets the placeholder', () {
      final account = _parse([_entry(name: null, issuer: null)]).accounts.single;
      expect(account.issuer, isNull);
    ...
    test('a blank issuer field falls through to the label issuer', () {
      final account =
          _parse([_entry(name: 'GitHub:alice', issuer: '   ')]).accounts.single;
      expect(account.issuer, 'GitHub');
    });
  ...
  group('malformed text fields', () {
    test('a non-UTF-8 name drops ONE entry, the rest of the QR still imports',
        () {
    ...
    test('a text field over maxStringBytes is a FormatException', () {
      expect(
          () => _parse([
                _entry(name: 'a' * (ProtobufLimits.maxStringBytes + 1)),
              ]),
          throwsFormatException);
    });
  ...
  group('SECURITY — no secret ever escapes', () {
    ...
    test('a structural FormatException names offsets, not bytes', () {
      ...
      final truncated =
          Uint8List.sublistView(payload, 0, payload.length - 3);
      Object? thrown;
      try {
  ...
  group('round trip', () {
    test('20 pseudo-random entries survive encode → URI → parse unchanged', () {
      ...
      for (var i = 0; i < 20; i++) {
        final secret =
            List<int>.generate(10 + next(23), (_) => next(256));
        ...
        entries.add(encodeOtpParameters(
          secret: secret,
          name: name,
          issuer: issuer,
      ...
      final uri = migrationUri(encodeMigrationPayload(
        entries: entries,
        batchSize: 1,
        batchIndex: 0,
        batchId: -7,
    ...
    test('accounts keep the payload order, and each gets a fresh id', () {
      ...
      expect(batch.accounts.map((a) => a.accountName),
