...
void main() {
  group('FeatureFlag.fromJson', () {
    ...
    test('enabled yanlış tip → FormatException', () {
      expect(
        () => FeatureFlag.fromJson({'key': 'x', 'enabled': 'yes'}),
  ...
  group('FeatureFlagsCacheStore', () {
    ...
    test('boş → null', () async {
      expect(
        await FeatureFlagsCacheStore(storage: _FakeStorage()).read(),
        isNull,
      );
    });
  });
  ...
  group('FeatureFlagsService', () {
    ...
    test('refresh → bellek hit (sunucu değeri)', () async {
      repo.flags = [
        const FeatureFlag(key: 'token_sync_enabled', enabled: false),
      ];
      final svc = FeatureFlagsService(repo: repo, cache: cache);
      await svc.refresh();
    ...
    test(
      'ensureLoaded: cache hit → fetch YOK + isEnabled cache değeri',
      () async {
    ...
    test(
      'ensureLoaded: cache boş + sunucu false → isEnabled false (kill-switch)',
      () async {
        repo.flags = [
          const FeatureFlag(key: 'token_sync_enabled', enabled: false),
        ];
    ...
    test(
      'ensureLoaded: cache boş + ağ hatası → fallback=true (sync açık)',
      () async {
        repo.throwError = true;
        final svc = FeatureFlagsService(repo: repo, cache: cache);
    ...
    test('listenable: flag değişince notify (refresh)', () async {
      repo.flags = [
        const FeatureFlag(key: 'token_sync_enabled', enabled: true),
      ];
      final svc = FeatureFlagsService(repo: repo, cache: cache);
      var notifications = 0;
