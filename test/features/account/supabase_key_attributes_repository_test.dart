...
void main() {
  group('SupabaseKeyAttributesRepository mapping', () {
    ...
    test('toRow → fromRow round-trip KAYIPSIZ (salt/ops/mem/emk/remk birebir)', () {
      ...
      expect(restored.encryptedMasterKey.nonce,
          original.encryptedMasterKey.nonce);
      expect(restored.encryptedMasterKey.ciphertext,
          original.encryptedMasterKey.ciphertext);
      expect(restored.recoveryEncryptedMasterKey.nonce,
          original.recoveryEncryptedMasterKey.nonce);
      expect(restored.recoveryEncryptedMasterKey.ciphertext,
    ...
    test('fromRow: eksik kolon → FormatException (fetch SyncMalformedRemote yapar)', () {
      final row = SupabaseKeyAttributesRepository.toRow('uid-A', _attrs());
      row.remove('recovery_nonce');
      expect(() => SupabaseKeyAttributesRepository.fromRow(row),
    ...
    test('bytea kolonları decode edilebilir (toRow çıktısı geçerli hex)', () {
      ...
      for (final k in [
        ...
        expect(() => ByteaCodec.decode(row[k] as String), returnsNormally,
