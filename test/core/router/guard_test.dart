...
void main() {
  group('guardRedirect', () {
    ...
    test(
      'uninitialized → setup ALT rotaları engellenir (mnemonic yok, review P1)',
      () {
        ...
        const s = VaultLockState.uninitialized();
        expect(
          guardRedirect(s, Routes.recoveryShow),
          Routes.setup,
        ); // /setup/recovery
        expect(
          guardRedirect(s, Routes.recoveryVerify),
          Routes.setup,
        ); // /setup/verify
      },
    );
    ...
    test(
      'setupPending → yalnız setup alt-ağacı; verify /setup\'a zıplatılmaz',
      () {
    ...
    test(
      'unlocked → vault + scan + settings serbest; auth ekranlarından vault\'a dön',
      () {
    ...
    test(
      'restoring → /splash (ASLA /setup — review [P1] #1) + hedefte null',
      () {
  ...
  group('sessionGuard (Faz 3 Patch 1 — kimlik kapısı)', () {
    ...
    test(
      'signedOut + /auth/link → /auth/login (link public DEĞİL, reviewer [P2])',
      () {
        final s = session(SessionStatus.signedOut);
    ...
    test('emailConfirmPending → /auth/confirm (trap; login geri atılır)', () {
      ...
      expect(sessionGuard(s, lockedVault, Routes.authConfirm), isNull);
      expect(
        sessionGuard(s, lockedVault, Routes.authLogin),
        Routes.authConfirm,
      );
    ...
    test(
      'signedIn + linkRequired → /auth/link (vault guard\'dan ÖNCE, bypass yok)',
      () {
    ...
    test(
      'signedIn + unlocked + /auth|/splash → vault (lock\'un istediği başlangıç)',
      () {
        final s = session(SessionStatus.signedIn);
    ...
    group('Patch 2 — restoring/restoreFailed (location korunur)', () {
      ...
      test(
        'signedIn + restoreFailed → /auth/restore-failed, HEDEFTE NULL (loop yok)',
        () {
          final s = session(SessionStatus.signedIn);
          expect(
            sessionGuard(s, restoreFailed, Routes.vault),
            Routes.authRestoreFailed,
          );
          expect(
            sessionGuard(s, restoreFailed, Routes.authLogin),
            Routes.authRestoreFailed,
          );
          ...
          expect(
            sessionGuard(s, restoreFailed, Routes.authRestoreFailed),
            isNull,
          );
        },
      );
      ...
      test(
        'signedIn + keyAttributesCorrupted → /auth-integrity, HEDEFTE NULL (regresyon)',
        () {
          final s = session(SessionStatus.signedIn);
          expect(
            sessionGuard(s, corrupted, Routes.vault),
            Routes.authIntegrity,
          );
