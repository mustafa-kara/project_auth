...
void main() {
  group('OtpAuthUri.parse', () {
    ...
    test('secret yoksa FormatException', () {
      expect(
        () => OtpAuthUri.parse('otpauth://totp/x?issuer=Y'),
    ...
    test('yanlış şema FormatException', () {
      expect(
        () => OtpAuthUri.parse('https://totp/x?secret=ABC'),
        throwsFormatException,
      );
    });
  });
  ...
  group(
    ...
    () {
      ...
      test(
        'parse edilen hesabın secretBytes\'ı her zaman güvenli decode olur',
        () {
          final a = OtpAuthUri.parse(
            'otpauth://totp/x?secret=JBSWY3DPEHPK3PXP',
          );
