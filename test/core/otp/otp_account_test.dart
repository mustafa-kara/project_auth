...
void main() {
  ...
  group('tags (Faz 5 Patch 3)', () {
    ...
    test(
      'normalizeTags: trim → boş at → 32 rune kırp → tekilleştir → ilk 8',
      () {
        // trim + boş atma
        ...
        expect(
          OtpAccount.normalizeTags(List.generate(12, (i) => 'etiket$i')).length,
          OtpAccount.maxTags,
        );
        // 32 rune kırpma
        final long = 'a' * 40;
        expect(
          OtpAccount.normalizeTags([long]).single.runes.length,
    ...
    test(
      'fromJson: "tags" List değil / eleman String değil → FormatException',
      () {
        expect(
    ...
    test('toString() etiketleri de BASMAZ (stringify kapalı)', () {
      expect(
        account().copyWith(tags: ['iş']).toString(),
        isNot(contains('iş')),
      );
    });
    ...
    test('normalizeTags çıktısı da DEĞİŞTİRİLEMEZ', () {
      ...
      expect(
        () => OtpAccount.normalizeTags(['iş']).add('ev'),
        throwsUnsupportedError,
      );
    });

    test(
      'normalizeTags: kullanıcı SIRASI korunur (alfabetik sıralama YOK)',
      () {
    ...
    test(
      'ctor: 40 karakterlik etiket 32 rune\'a kırpılır (import grup adı)',
      () {
    ...
    test(
      'fromJson: depodaki KİRLİ etiket listesi normalize edilir, atılmaz',
      () {
    ...
    test(
      'props: etiket SIRASI da eşitliğe girer (liste derin karşılaştırma)',
      () {
        expect(
          account().copyWith(tags: ['iş', 'ev']),
