...
- [x] Dependencies added + versions resolved: `flutter_bloc 9.1`, `go_router 17.3`, `get_it 9.2`, `supabase_flutter 2.17`, `sodium_libs 3.4`, `flutter_secure_storage 10.3`, `mobile_scanner 7.4`, `local_auth 3.0`, `equatable`, `uuid`, `crypto`. ✅
  *(2026-09-01: `injectable`/`injectable_generator`, `freezed`/`freezed_annotation`, `json_annotation`/`json_serializable`, `build_runner` and `bloc_test` were REMOVED — never used. DI is a hand-written `get_it` composition root, JSON is hand-written, `mocktail` is the only mock library.)*
  ...
  - **Deferred major upgrades (NOT pins — no compatibility blocker, each just needs its own migration):** `go_router` 18, `flutter_secure_storage` 11. `device_info_plus` 13 and `file_picker` 12 are **one coupled job, not two**: `file_picker >=12.1.3` pulls `windows_file_picker` → `win32 ^6.3.0` while `device_info_plus ^12.1.0` requires `win32 ^5.11.0`, so neither resolves alone (see the pubspec comment and docs/architecture.md §7).
...
- [ ] **iOS release-build check — `NSPhotoLibraryUsageDescription`.** `file_picker 11.0.3`'s iOS podspec pulls `DKImagePickerController/PhotoGallery` whenever `PICKER_MEDIA` is left at its default (see `ios/Podfile.lock`), which links photo-library APIs into the binary even though this app only ever picks documents. `ios/Runner/Info.plist` declares camera and Face ID strings but **no** photo-library string, so App Store review can reject the upload. Two ways out, decide before the first submission: set `PICKER_MEDIA=false` in `ios/Podfile` (drops `DKImagePickerController`, `SDWebImage`, `SwiftyGif` entirely — preferred, the app never needs the gallery), or add the usage string.
