...
void main() {
  ...
  group('encrypted save → load', () {
    ...
    test(
      'raw storage güvenlik: secret/issuer/accountName plaintext geçmez',
      () async {
        final s = FakeSecureStorage();
  ...
  group('unchanged-blob koruması', () {
    test(
      'aynı içerikle iki save → değişmeyen kayıt yeniden şifrelenmez',
      () async {
        final s = FakeSecureStorage();
        final r = repo(s);
        final a = acc('sabit');
  ...
  group('bozuk kayıt davranışı', () {
    ...
    test(
      'bozuk tek record atlanır + corruptedCount==1, diğerleri yüklenir',
      () async {
    ...
    test(
      'purgeCorrupted: bozuk silinir, sağlam+unchanged blob korunur',
      () async {
        final s = await withCorrupted([acc('a'), acc('b')]);
        final r = repo(s);
        ...
        final aBefore = listBefore.firstWhere(
          (e) => e['id'] == before.accounts[0].id,
        );
        ...
        expect(
          listAfter.any((e) => e['id'] == 'corrupt-1'),
          isFalse,
          reason: 'bozuk silinmeli',
        );
        expect(listAfter.length, 2);
        final aAfter = listAfter.firstWhere(
          (e) => e['id'] == before.accounts[0].id,
        );
    ...
    test(
      'non-map bozuk kayıt (scalar/null) korunur, save TypeError atmaz',
      () async {
  ...
  group('bütünlük (sessiz veri kaybı yok)', () {
    ...
    test(
      'tüm kayıtlar decrypt fail (yanlış masterKey) → VaultIntegrityException',
      () async {
        final s = FakeSecureStorage();
        await repo(s).save([acc('a'), acc('b')]);
  ...
  group('migration (Faz 1 plaintext → şifreli)', () {
    ...
    test(
      'plaintext → encrypted; plaintext silinir; marker committed; secret yok',
      () async {
        final s = FakeSecureStorage();
    ...
    test(
      'crash idempotency: encrypted yazıldı + plaintext duruyor + marker yok → tekrar çalışır, duplicate yok',
      () async {
        final s = FakeSecureStorage();
    ...
    test(
      'upsert: encrypted\'te FARKLI kayıt varken migration onu EZMEZ (merge)',
      () async {
    ...
    test(
      'upsert: çakışan id → mevcut şifreli kayıt korunur (duplicate yok)',
      () async {
        final s = FakeSecureStorage();
        ...
        final sharedLoaded = loaded.accounts.firstWhere(
          (a) => a.id == shared.id,
        );
