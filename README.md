...
6. **Admin panel** — **MVP DONE 2026-09-02:** Next.js 16 panel under `admin/` — global counts, user ban/unban/delete, announcements/catalog/feature-flag CRUD, audit log viewer, every privileged operation audited. FCM push triggering stays in Phase 4 (needs the Firebase project + device push tokens); the `admin_backend`/`admin_app` DB role migration was **applied** to the live project on 2026-09-02 and both roles now exist — what is left is operator-only (the `admin_app` password, the secret key, and the first `public.admin_users` row)
...
> `admin_app` login role was created by hand afterwards and granted `admin_backend`, but its **password is
> still unset**: the operator sets it with `alter role admin_app password '…';` in the Dashboard SQL editor
> (never in a migration or a transcript), and the role cannot connect until then. See
...
> 2026-09-02.
>
> **What remains is operator-only:** set the `admin_app` password (`alter role admin_app password '…';` — the
> role has none yet and cannot connect until it does) and put that role into `DATABASE_URL`, and paste the
> `sb_secret_…` key. Until then the stats cards render an error card and every other page works normally. You
> also need at least one row in `public.admin_users`, or nobody can get past `/login` — and there is none yet,
> because `auth.users` currently holds only a UI-test account.
