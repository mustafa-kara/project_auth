...
(client + server), plus the **server-only** `SUPABASE_SECRET_KEY` (`sb_secret_…`) and `DATABASE_URL` (the
`admin_app` login role). The schema rejects legacy `eyJ…` JWT anon/service_role keys outright. Full table,
the operator SQL for the DB role, the three access paths and the module contract:
[admin/README.md](admin/README.md).

> ⏳ **Before the dashboard shows numbers:** apply `supabase/migrations/20260902120000_admin_backend_role.sql`
> and create the `admin_app` login role (the migration carries no password on purpose). Until then the stats
> cards render an error card and every other page works normally. You also need at least one row in
> `public.admin_users`, or nobody can get past `/login`.
